!==============================================================================
subroutine dftd(natoms, ngroups, xyz, tvec, iz, ilist, imat, cn, dcn, &
                func, version,                                        &
                                                        disp, dispgrad)
  !----------------------------------------------------------------------------
  !
  ...
  !
  INTEGER, INTENT(in), DIMENSION(natoms)                          :: iz
  INTEGER, INTENT(in), DIMENSION(natoms)                          :: ilist
  INTEGER, INTENT(in), DIMENSION(ngroups)                         :: imat
  !
  DOUBLE PRECISION,INTENT(in),DIMENSION(3)                        :: tvec
  DOUBLE PRECISION,INTENT(in),DIMENSION(3,natoms)                 :: xyz
  DOUBLE PRECISION,INTENT(in),DIMENSION(natoms)                   :: cn
  DOUBLE PRECISION,INTENT(in),DIMENSION(3,natoms,natoms)          :: dcn
  !
  CHARACTER*8, INTENT(in)                                         :: func
  !
  !----------------------------------------------------------------------------
  !
  DOUBLE PRECISION,INTENT(out)                                    :: disp
  DOUBLE PRECISION,INTENT(out),optional, DIMENSION(3,natoms)      :: dispgrad
  !
  !----------------------------------------------------------------------------
  !
  DOUBLE PRECISION, PARAMETER      :: au_2_ang = 0.52917726d0                 &
                                    , c6conv = 1.0d-3/2625.4999d0/((0.052917726d0)**6)
  !
  DOUBLE PRECISION, PARAMETER      :: k1 =  16.0d0                            &
                                    , k2 =  4.0d0 / 3.0d0                     &
                                    , k3 = -4.0d0
  !
  !----------------------------------------------------------------------------
  !
  INTEGER, DIMENSION(94)                                     :: max_c
  !
  DOUBLE PRECISION                                           :: s6            &
                                                              , rs6           &
                                                              , s8            &
                                                              ...
                                                              , alp8
  !
  DOUBLE PRECISION                                           :: gdsp
  !
  DOUBLE PRECISION, DIMENSION(94)                            :: r2_r4
  DOUBLE PRECISION, DIMENSION(94,94)                         :: r0_ab
  DOUBLE PRECISION, DIMENSION(94,94,5,5,3)                   :: c6_ab
  !
  !----------------------------------------------------------------------------
  !
  ! call for functional dependend parameters
  call func_parameters(func, version                                          &
                      , s6, rs6, s8, rs8, alp6, alp8)
  !
  IF (version .eq. 3) THEN
    !
    ! Prepare other atomic radii parameters
    call r2r4_parameters(r2_r4)
    !
    ! prepare pairwise distance parameters
    call r0ab_parameters(r0_ab)
    !
    ! get c6 parameters
    call copy_c6(5,94,c6_ab,max_c)
    !
  ELSE IF (version .eq. 2) THEN
    !
    call d2_paramaters(94,5,c6_ab,r0_ab)
    !
    c6_ab = c6_ab * c6conv
    !
    max_c = 1
  ...
  !
  ! Energy call
  call e_disp(94,5,natoms,ngroups,xyz,tvec,iz,ilist,imat,cn,                  &
              max_c,c6_ab,r2_r4,r0_ab,s6,s8,rs6,rs8,alp6,alp8,version,        &
              disp)
  !
  IF (present(dispgrad)) THEN
    !
    !
    ! gradient call
    call g_disp(94,5,natoms,ngroups,xyz,tvec,iz,ilist,imat,cn,dcn             &
               ,max_c,c6_ab,r2_r4,r0_ab,s6,s8,rs6,rs8,alp6,alp8,version       &
               ,.false.                                                       &
               ,gdsp, dispgrad)
    !
...
!
!==============================================================================
subroutine func_parameters( func, version                                     &
                          , s6, rs6, s8, rs8, alp6, alp8)
  !----------------------------------------------------------------------------
  !
  implicit none
  ...
  !----------------------------------------------------------------------------
  !
  DOUBLE PRECISION, INTENT(out)                              :: s6
  DOUBLE PRECISION, INTENT(out)                              :: rs6
  DOUBLE PRECISION, INTENT(out)                              :: s8
  DOUBLE PRECISION, INTENT(out)                              :: rs8
  DOUBLE PRECISION, INTENT(out)                              :: alp6
  DOUBLE PRECISION, INTENT(out)                              :: alp8
  !
  !----------------------------------------------------------------------------
  !
  if (version .eq. 3) then
  ...
  !
  !----------------------------------------------------------------------------
end subroutine func_parameters
!==============================================================================
!
!==============================================================================
subroutine r2r4_parameters(r2_r4)
  !----------------------------------------------------------------------------
  !
  implicit none
  !
  !----------------------------------------------------------------------------
  DOUBLE PRECISION, INTENT(out), DIMENSION(94)               :: r2_r4
  !----------------------------------------------------------------------------
  INTEGER                                                    :: ind
  !----------------------------------------------------------------------------
  r2_r4 = (/                                                                      &
  ...
  !
  do ind = 1,94
    r2_r4(ind) = sqrt(0.5d0 * r2_r4(ind) * sqrt(dfloat(ind)))
  end do
  !
  !----------------------------------------------------------------------------
end subroutine r2r4_parameters
!==============================================================================
!
!==============================================================================
subroutine r_cov_parameters(r_cov)
  !----------------------------------------------------------------------------
  !
  implicit none
  !
  !----------------------------------------------------------------------------
  DOUBLE PRECISION, INTENT(out), DIMENSION(94)               :: r_cov
  !----------------------------------------------------------------------------
  r_cov = (/                                                                      &
  ...
  !----------------------------------------------------------------------------
end subroutine r_cov_parameters
!==============================================================================
!
!==============================================================================
subroutine r0ab_parameters(r0_ab)
  !----------------------------------------------------------------------------
  implicit none
  !----------------------------------------------------------------------------
  !
  DOUBLE PRECISION, INTENT(out), DIMENSION(94,94)            :: r0_ab
  !
  !----------------------------------------------------------------------------
  !
  INTEGER                                                    :: i,j,k
  !
  DOUBLE PRECISION, DIMENSION(4465)                          :: r0ab
  !
  !----------------------------------------------------------------------------
  !
  r0ab(   1:  70)=(/  &
  ...
  do i=1,94
     do j=1,i
        k=k+1
        r0_ab(i,j)=r0ab(k)/0.52917726d0
        r0_ab(j,i)=r0ab(k)/0.52917726d0
     enddo
  enddo
  !----------------------------------------------------------------------------
end subroutine r0ab_parameters
!==============================================================================
